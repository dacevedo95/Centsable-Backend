...
def get_token():
    ...
    if not country_code:
        return jsonify({'msg': 'Missing country code parameter'}), 400

    # Tries to find a user by the given email
    user = User.query.filter(User.country_calling_code == country_code, User.phone_number == phone_number).first()
